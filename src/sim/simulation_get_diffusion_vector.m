...
num_tris = size(tri4, 2);
    ...
    elseif isfield(data,'diff_map');
        ...
        figure; imshow(data.diff_map); title('Diffusion Map'); colorbar;
        hold on;
%          if data.orientation ~= []
%             plot(data.orientation(1:2,1),data.orientation(1:2,2),'w-');
%             plot(data.orientation(3:4,1),data.orientation(3:4,2),'w-');
%          end
        ...
        elseif strcmp(cell_name,'tensor_diffusion');
            ...
            D = {(reshape(data.D{1},4,1));(reshape(data.D{2},4,1))};
            cartesian_diff_const = {[data.diff_const(1),0,data.diff_const(1)], ...
                [D{1}(1),D{1}(2),D{1}(4)], [D{2}(1),D{2}(2),D{2}(4)]};
        ...
        elseif strcmp(cell_name,'tensor_cross'),
            ...
            diff_tag = double(diff_coef(:,1)<=200-128) + 2*double(diff_coef(:,1)>200-128 & diff_coef(:,1)<=215-128)+...
...
%                diff_tag = diff_tag+i.* double(diff_coef==data.diff_const(i));
                %%% 03/31/2015
